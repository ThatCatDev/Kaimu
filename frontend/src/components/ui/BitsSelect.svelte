...
<div class="w-full">
  ...
  {#if readOnly}
    <p class="text-sm text-gray-900">{selectedLabel || '—'}</p>
  {:else}
    <Select.Root type="single" {value} onValueChange={handleValueChange} {disabled}>
    <Select.Trigger
      {id}
      class="flex h-auto w-full items-center justify-between rounded bg-transparent px-2 py-1.5 text-sm transition-colors hover:bg-gray-50 focus:outline-none focus:bg-gray-50 focus:ring-1 focus:ring-indigo-500 disabled:cursor-not-allowed disabled:opacity-50 {error ? 'ring-1 ring-red-500 bg-red-50' : ''}"
    >
